...
- **`performAccessibilityAudit` on macOS needs two false-positives filtered** — `runDescriptionAndContrastAudit` in `AccessibilityTests` collects issues (returning `true` from the closure = "handled") and asserts with a single-line message so the element shows in Xcode's inline error popup. It ignores: (a) the `NavigationSplitView` structural container Group/SplitGroup (`elementType == .group/.splitGroup` or desc contains "NavigationSplitView"/"SplitGroup") — a layout container with no description by design; (b) the macOS **Touch Bar** emoji/symbols "Candidate Bar" (`desc.contains("TouchBar")`) — system UI injected into the AX tree, not ours. Use `issue.auditType.contains(.sufficientElementDescription)`, NOT `==` (it's an OptionSet).
