...
- **Archive build-number auto-increment uses `sed`, NOT `agvtool` — and `VERSIONING_SYSTEM` must stay UNSET (DECIDED, do not re-add either)**: the Archive action has a pre-action script (in `ElapsedTimeAdder.xcscheme`) that stamps a fresh date-based build number (`$(date +"%y%m%d%H%M")`) into `CURRENT_PROJECT_VERSION` before every archive, via `sed -i '' "s/CURRENT_PROJECT_VERSION = [0-9]*;/CURRENT_PROJECT_VERSION = ${BUILD_NUMBER};/g" ElapsedTimeAdder.xcodeproj/project.pbxproj`. This replaced an original `agvtool new-version -all` call, which had two stacked problems: (1) `agvtool` silently no-ops unless the project's Versioning System build setting is `apple-generic`; (2) setting `VERSIONING_SYSTEM = "apple-generic"` to fix that made things *worse* — it doesn't just enable the CLI tool, it also makes **Xcode's own build system automatically invoke agvtool internally on every build**, completely independent of our scheme script. That internal invocation is what triggered a persistent, unresolved macOS "the 'agvtool' command requires command line developer tools" dialog during Archive — it kept recurring even after the script itself was rewritten to use `sed` and stopped calling `agvtool` at all, which is what proved the dialog was coming from Xcode's internal versioning step, not our script. (The CLI tool itself was separately confirmed fully functional — exit 0, correctly updates the file — when run manually from a plain shell; CLT reinstall, `sudo xcodebuild -runFirstLaunch`, and the Privacy & Security → Developer Tools permission were all ruled out as the cause.) Fix: removed `VERSIONING_SYSTEM` from `project.pbxproj` entirely (was only ever needed for the now-abandoned `agvtool` approach) in addition to using `sed` in the script. If archiving ever stops bumping the build number, or that dialog reappears, check for (a) the scheme's Archive pre-action script having been reverted back to `agvtool`, or (b) `VERSIONING_SYSTEM` having been re-added (e.g. via Xcode's GUI "Versioning" build setting UI).
