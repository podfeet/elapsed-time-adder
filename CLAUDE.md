...
- **App display names (DECIDED — do NOT rename `PRODUCT_NAME` on macOS):**
  - iOS home screen → **"Time Adder"**: `INFOPLIST_KEY_CFBundleDisplayName = "Time Adder";` (both configs).
  - macOS Finder / Applications → **"Elapsed Time Adder"**: `"INFOPLIST_KEY_CFBundleDisplayName[sdk=macosx*]" = "Elapsed Time Adder";` (both configs). This is harmless and works.
  - macOS Dock / ⌘-Tab / menu bar → stuck at **"ElapsedTimeAdder"** (no spaces). These read `CFBundleName` (= `PRODUCT_NAME`), NOT `CFBundleDisplayName`, and `INFOPLIST_KEY_CFBundleName` is ignored under `GENERATE_INFOPLIST_FILE = YES`. The window title is **hidden** via `.toolbar(.hidden, for: .windowToolbar)` on the macOS `NavigationSplitView`, so the no-spaces name isn't shown prominently.
  - **The ONLY lever for the Dock/menu-bar name is `PRODUCT_NAME[sdk=macosx*] = "Elapsed Time Adder"` — and it was TRIED and REVERTED because it cascades into test-infrastructure breakage:** it renames the macOS `.app` (needs a `TEST_HOST[sdk=macosx*]` override), renames the Swift module to `Elapsed_Time_Adder` (breaks `@testable import ElapsedTimeAdder` unless `PRODUCT_MODULE_NAME = ElapsedTimeAdder` is pinned), and even with both fixes the **unit-test target silently fails to compile/discover** (logic tests stop running — worse than a visible failure). Do NOT re-attempt the `PRODUCT_NAME` rename without a fully verified solution. The spaced-Dock-name is an accepted future task, not a quick fix.
