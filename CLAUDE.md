...
- **Archive build-number auto-increment uses a build-phase script that patches the BUILT Info.plist directly, NOT a scheme pre-action, NOT `agvtool`, and `VERSIONING_SYSTEM` must stay UNSET (DECIDED, do not re-add any of those).** Current mechanism: `Scripts/set-build-number.sh`, wired in as a `PBXShellScriptBuildPhase` named "Set Build Number" on the main target (declares `$(TARGET_BUILD_DIR)/$(INFOPLIST_PATH)` as an input file, forcing it to run after `ProcessInfoPlistFile`, same trick as `set-macos-bundle-name.sh`). It stamps `CFBundleVersion` with `$(date +"%y%m%d%H%M")` directly on the archived Info.plist. History of two prior failed approaches, in order:
  1. **`agvtool new-version -all`** — silently no-ops unless `VERSIONING_SYSTEM = apple-generic`; setting that made Xcode's build system auto-invoke agvtool internally on *every* build, which triggered a persistent, unresolved "the 'agvtool' command requires command line developer tools" dialog during Archive (confirmed coming from Xcode's internal versioning step, not any of our scripts — CLT reinstall, `sudo xcodebuild -runFirstLaunch`, and Developer Tools privacy permission were all ruled out).
  2. **A scheme Archive pre-action running `sed -i '' "s/CURRENT_PROJECT_VERSION = [0-9]*;/.../"` against `project.pbxproj`** — fixed the agvtool dialog, but had its own hidden bug: the pre-action edits `project.pbxproj` from outside Xcode's process while Xcode still has the project open with its own in-memory build-settings model. Xcode doesn't reliably reload that external edit before starting the Archive build, so the archive could embed a `CURRENT_PROJECT_VERSION` that was stale by *days*, even though the sed had correctly written a fresh value to the file on disk moments earlier. Confirmed directly: an archive created 2026-07-28 embedded build number `2607251826` (2026-07-25) despite `project.pbxproj` on disk already showing the correct 2026-07-28 value at archive time.
  - **Current fix (build-phase script, not pre-action)**: patches the already-*built* Info.plist output after Xcode has finished its own plist processing, so it never depends on Xcode noticing/reparsing an externally-edited source file — there's no project-model cache to go stale. This also means `project.pbxproj`'s `CURRENT_PROJECT_VERSION` is no longer touched by archiving at all (it's dead/unused now — don't bother keeping it in sync) and archiving no longer leaves an uncommitted `project.pbxproj` diff.
  - If build numbers ever go stale again, check: (a) the "Set Build Number" build phase still exists on the main target and still declares the Info.plist as an input file, (b) nobody re-added a scheme pre-action or `agvtool`, (c) `VERSIONING_SYSTEM` hasn't been re-added via Xcode's GUI "Versioning" build setting.
